...
import { chatActions } from "@/store/reducer/chat-reducer/chat";
...
const GoogleDriveAttachments = ({
  attachment,
}: IGoogleDriveAttachmentsProps) => {
  ...
  return (
    <div
      ...
      className={cn(
        "cursor-pointer flex items-center border rounded-xl p-5 py-3 gap-5",
        {
          "bg-[#F7F7F7]":
            selectedAttachment?.documentId === attachment.documentId,
        }
      )}
    >
      <div>
        <Image src={DriveImage} alt="google drive" height={40} width={40} />
      </div>
