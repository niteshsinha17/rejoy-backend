...
import {
  ...
  IAgentSendMessageServicePayload,
} from "@/models/chat";
...
export const agentApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    addAgentMemory: builder.mutation<any, IAgentAddMemoryServicePayload>({
      query: ({ type, url, agentId }) =>
        apiObject(`agent/${agentId}/memory/add/`).post({ type, url }),
      invalidatesTags: (result, error, { agentId }) => [
        { type: "AgentMemoryList", id: agentId },
      ],
    }),
    deleteAgentMemory: builder.mutation<void, IAgentDeleteMemoryServicePayload>(
      {
        query: ({ agentId, memoryId }) =>
          apiObject(`agent/${agentId}/memory/${memoryId}/delete/`).delete(),
        invalidatesTags: (result, error, { agentId }) => [
          { type: "AgentMemoryList", id: agentId },
        ],
      }
    ...
    updateAgentGoogleAccount: builder.mutation<
      undefined,
      IAgentUpdateGoogleLoginServicePayload
    >({
      query: ({ agentId, ...payload }) =>
        apiObject(`agent/${agentId}/google-account/update/`).post(payload),
    }),
    invitePerson: builder.mutation<undefined, IAgentPersonInviteServicePayload>(
      {
        query: ({ agentId, ...payload }) =>
          apiObject(`agent/${agentId}/invite-person/`).post(payload),
        invalidatesTags: (result, error, { agentId }) => [
          { type: "inviteeList", id: agentId },
        ],
      }
    ),
    ...
    agentIntakePersonList: builder.query<
      IAgentIntakePersonListItemResponse[],
      string
    >({
      query: (agentId) => apiObject(`agent/${agentId}/intake-person/list/`),
      providesTags: (result, error, id) => [
        { type: "AgentIntakePersonList", id },
      ],
    }),
    removeInvitePerson: builder.mutation<
      undefined,
      IRemoveInvitePersonServicePayload
    >({
      query: ({ inviteId, agentId }) =>
        apiObject(
          `agent/${agentId}/invite-person/${inviteId}/delete/`
        ).delete(),
      invalidatesTags: (result, error, { agentId }) => [
        { type: "inviteeList", id: agentId },
      ],
    }),
    removeIntakePerson: builder.mutation<
      undefined,
      IRemoveIntakePersonServicePayload
    >({
      query: ({ intakePersonId, agentId }) =>
        apiObject(
          `agent/${agentId}/intake-person/${intakePersonId}/delete/`
        ).delete(),
      invalidatesTags: (result, error, { agentId }) => [
        { type: "AgentIntakePersonList", id: agentId },
      ],
    }),
    disabledResponse: builder.mutation<
      undefined,
      IAgentDisableResponseServicePayload
    >({
      query: ({ agentId, data }) =>
        apiObject(`agent/${agentId}/disable-response/`).post(data),
      onQueryStarted(payload, api) {
        const { dispatch, queryFulfilled } = api;
        queryFulfilled.then(() => {
          dispatch(
