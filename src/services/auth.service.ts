...
import {
  ICreateGuestUserServicePayload,
  ILoginServiceErrorResponse,
...
export const authenticateTokenService = () =>
  baseApiQuery<ILoginServiceResponse>(
    apiObject("auth/check-auth/").doNotRedirect()
  );

export const loginWithCredentialsService = (
  username: string,
  password: string
) =>
  baseApiQuery<ILoginServiceResponse, ILoginServiceErrorResponse>(
    apiObject("auth/login/").post({ username, password })
  );

export const createUser = (username: string, password: string) =>
  baseApiQuery<ILoginServiceResponse>(
    apiObject("auth/create-user/").post({ username, password })
  );

export const isUsernameAvailableService = (username: string) =>
  baseApiQuery(apiObject("auth/is-username-available/").post({ username }));
...
export const authApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    >({
      query: (payload) =>
        apiObject("auth/send-phone-verification-otp/").post(payload),
    }),
    ...
    sendEmailVerificationOtp: builder.mutation<undefined, string>({
      query: (email) =>
        apiObject("auth/send-email-verification-code/").post({ email }),
    }),
